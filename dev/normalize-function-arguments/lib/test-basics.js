(async function() {
  'use strict';
  var GTNG, GUY, Test, alert, debug, echo, f, help, info, inspect, log, plain, praise, reverse, rpr, urge, warn, whisper,
    splice = [].splice;

  GUY = require('guy');

  ({alert, debug, help, info, plain, praise, urge, warn, whisper} = GUY.trm.get_loggers('intertype/test-basics'));

  ({rpr, inspect, echo, reverse, log} = GUY.trm);

  // WGUY                      = require '../../../apps/webguy'
  GTNG = require('../../../apps/guy-test-NG');

  ({Test} = GTNG);

  ({f} = require('../../../apps/effstring'));

  //###########################################################################################################

  //===========================================================================================================
  this.nfa_tasks = {
    //=========================================================================================================
    basics: function() {
      var NFA, get_signature, nfa, red;
      NFA = require('../../../apps/normalize-function-arguments');
      ({nfa, get_signature} = NFA);
      ({red, reverse} = GUY.trm);
      (() => {        //.......................................................................................................
        var fn, frz, Ωnfat__10, Ωnfat__11, Ωnfat__12, Ωnfat__13, Ωnfat__14, Ωnfat__15, Ωnfat__16, Ωnfat__17, Ωnfat__18, Ωnfat__19, Ωnfat__20, Ωnfat___1, Ωnfat___2, Ωnfat___3, Ωnfat___4, Ωnfat___5, Ωnfat___6, Ωnfat___7, Ωnfat___8, Ωnfat___9;
        fn = nfa(function(a, b, c, cfg) {
          return {a, b, c, cfg};
        });
        frz = Object.freeze;
        //.....................................................................................................
        this.eq((Ωnfat___1 = function() {
          return fn();
        }), {
          a: void 0,
          b: void 0,
          c: void 0,
          cfg: {}
        });
        this.eq((Ωnfat___2 = function() {
          return fn(1);
        }), {
          a: 1,
          b: void 0,
          c: void 0,
          cfg: {
            a: 1
          }
        });
        this.eq((Ωnfat___3 = function() {
          return fn(1, 2);
        }), {
          a: 1,
          b: 2,
          c: void 0,
          cfg: {
            a: 1,
            b: 2
          }
        });
        this.eq((Ωnfat___4 = function() {
          return fn(1, 2, 3);
        }), {
          a: 1,
          b: 2,
          c: 3,
          cfg: {
            a: 1,
            b: 2,
            c: 3
          }
        });
        this.throws((Ωnfat___5 = function() {
          return fn(1, 2, 3, 4);
        }), /expected up to 3 positional arguments, got 4/);
        this.eq((Ωnfat___6 = function() {
          return fn({});
        }), {
          a: void 0,
          b: void 0,
          c: void 0,
          cfg: {}
        });
        this.eq((Ωnfat___7 = function() {
          return fn(1, {});
        }), {
          a: 1,
          b: void 0,
          c: void 0,
          cfg: {
            a: 1
          }
        });
        this.eq((Ωnfat___8 = function() {
          return fn(1, 2, {});
        }), {
          a: 1,
          b: 2,
          c: void 0,
          cfg: {
            a: 1,
            b: 2
          }
        });
        this.eq((Ωnfat___9 = function() {
          return fn(1, 2, 3, {});
        }), {
          a: 1,
          b: 2,
          c: 3,
          cfg: {
            a: 1,
            b: 2,
            c: 3
          }
        });
        this.throws((Ωnfat__10 = function() {
          return fn(1, 2, 3, 4, {});
        }), /expected up to 3 positional arguments, got 4/);
        this.eq((Ωnfat__11 = function() {
          return fn({
            b: 88
          });
        }), {
          a: void 0,
          b: 88,
          c: void 0,
          cfg: {
            b: 88
          }
        });
        this.eq((Ωnfat__12 = function() {
          return fn(1, {
            b: 88
          });
        }), {
          a: 1,
          b: 88,
          c: void 0,
          cfg: {
            b: 88,
            a: 1
          }
        });
        this.eq((Ωnfat__13 = function() {
          return fn(1, 2, {
            b: 88
          });
        }), {
          a: 1,
          b: 2,
          c: void 0,
          cfg: {
            b: 2,
            a: 1
          }
        });
        this.eq((Ωnfat__14 = function() {
          return fn(1, 2, 3, {
            b: 88
          });
        }), {
          a: 1,
          b: 2,
          c: 3,
          cfg: {
            b: 2,
            a: 1,
            c: 3
          }
        });
        this.throws((Ωnfat__15 = function() {
          return fn(1, 2, 3, 4, {
            b: 88
          });
        }), /expected up to 3 positional arguments, got 4/);
        this.eq((Ωnfat__16 = function() {
          return fn(frz({
            b: 88
          }));
        }), {
          a: void 0,
          b: 88,
          c: void 0,
          cfg: {
            b: 88
          }
        });
        this.eq((Ωnfat__17 = function() {
          return fn(1, frz({
            b: 88
          }));
        }), {
          a: 1,
          b: 88,
          c: void 0,
          cfg: {
            b: 88,
            a: 1
          }
        });
        this.eq((Ωnfat__18 = function() {
          return fn(1, 2, frz({
            b: 88
          }));
        }), {
          a: 1,
          b: 2,
          c: void 0,
          cfg: {
            b: 2,
            a: 1
          }
        });
        this.eq((Ωnfat__19 = function() {
          return fn(1, 2, 3, frz({
            b: 88
          }));
        }), {
          a: 1,
          b: 2,
          c: 3,
          cfg: {
            b: 2,
            a: 1,
            c: 3
          }
        });
        this.throws((Ωnfat__20 = function() {
          return fn(1, 2, 3, 4, frz({
            b: 88
          }));
        }), /expected up to 3 positional arguments, got 4/);
        //.....................................................................................................
        return null;
      })();
      (() => {        //.......................................................................................................
        var fn, frz, Ωnfat__21, Ωnfat__22, Ωnfat__23, Ωnfat__24, Ωnfat__25, Ωnfat__26, Ωnfat__27, Ωnfat__28;
        fn = nfa(function(cfg) {
          return {cfg};
        });
        frz = Object.freeze;
        //.....................................................................................................
        this.eq((Ωnfat__21 = function() {
          return fn();
        }), {
          cfg: {}
        });
        this.throws((Ωnfat__22 = function() {
          return fn(1, 2, 3, 4);
        }), /expected up to 0 positional arguments, got 4/);
        this.eq((Ωnfat__23 = function() {
          return fn({});
        }), {
          cfg: {}
        });
        this.throws((Ωnfat__24 = function() {
          return fn(1, 2, 3, 4, {});
        }), /expected up to 0 positional arguments, got 4/);
        this.eq((Ωnfat__25 = function() {
          return fn({
            b: 88
          });
        }), {
          cfg: {
            b: 88
          }
        });
        this.throws((Ωnfat__26 = function() {
          return fn(1, 2, 3, 4, {
            b: 88
          });
        }), /expected up to 0 positional arguments, got 4/);
        this.eq((Ωnfat__27 = function() {
          return fn(frz({
            b: 88
          }));
        }), {
          cfg: {
            b: 88
          }
        });
        this.throws((Ωnfat__28 = function() {
          return fn(1, 2, 3, 4, frz({
            b: 88
          }));
        }), /expected up to 0 positional arguments, got 4/);
        //.....................................................................................................
        return null;
      })();
      (() => {        //.......................................................................................................
        var fn, frz, Ωnfat__29, Ωnfat__30, Ωnfat__31, Ωnfat__32, Ωnfat__33, Ωnfat__34, Ωnfat__35, Ωnfat__36;
        fn = nfa(function() {
          return {};
        });
        frz = Object.freeze;
        //.....................................................................................................
        this.eq((Ωnfat__29 = function() {
          return fn();
        }), {});
        this.throws((Ωnfat__30 = function() {
          return fn(1, 2, 3, 4);
        }), /expected up to 0 positional arguments, got 4/);
        this.throws((Ωnfat__31 = function() {
          return fn({});
        }), /expected up to 0 named arguments objects, got 1/);
        this.throws((Ωnfat__32 = function() {
          return fn(1, 2, 3, 4, {});
        }), /expected up to 0 named arguments objects, got 1/);
        this.throws((Ωnfat__33 = function() {
          return fn({
            b: 88
          });
        }), /expected up to 0 named arguments objects, got 1/);
        this.throws((Ωnfat__34 = function() {
          return fn(1, 2, 3, 4, {
            b: 88
          });
        }), /expected up to 0 named arguments objects, got 1/);
        this.throws((Ωnfat__35 = function() {
          return fn(frz({
            b: 88
          }));
        }), /expected up to 0 named arguments objects, got 1/);
        this.throws((Ωnfat__36 = function() {
          return fn(1, 2, 3, 4, frz({
            b: 88
          }));
        }), /expected up to 0 named arguments objects, got 1/);
        //.....................................................................................................
        return null;
      })();
      (() => {        //.......................................................................................................
        var optional, Ωnfat__37, Ωnfat__38, Ωnfat__39, Ωnfat__40, Ωnfat__41, Ωnfat__43, Ωnfat__44;
        optional = null;
        
      const empty_fn = function (

        ) {};
      ;
        this.eq((Ωnfat__37 = function() {
          return get_signature(function(a) {});
        }), {
          a: 'bare'
        });
        this.throws((Ωnfat__38 = function() {
          return get_signature(function(a = optional) {});
        }), /not compliant/);
        this.eq((Ωnfat__39 = function() {
          return get_signature(empty_fn);
        }), {});
        this.eq((Ωnfat__40 = function() {
          return get_signature(function() {});
        }), {});
        this.eq((Ωnfat__41 = function() {
          return get_signature(function(a, b, c) {});
        }), {
          a: 'bare',
          b: 'bare',
          c: 'bare'
        });
        // ### TAINT limitation of CoffeeScript: signature runs up to soak, trailing paramters handled inside function body ###
        // @eq     ( Ωnfat__42 = -> get_signature ( a, b..., c               ) ->  ), { a: 'bare', b: 'soak', }
        this.throws((Ωnfat__43 = function() {
          return get_signature(function(a, ...b) {
            var c, ref;
            ref = b, [...b] = ref, [c] = splice.call(b, -1);
          });
        }), /not compliant/);
        return this.throws((Ωnfat__44 = function() {
          return get_signature(function(a, b = null) {});
        }), /not compliant/);
      })();
      //.......................................................................................................
      return null;
    }
  };

  //===========================================================================================================
  if (module === require.main) {
    await (() => {
      var guytest_cfg;
      guytest_cfg = {
        throw_on_error: false,
        show_passes: false,
        report_checks: false
      };
      guytest_cfg = {
        throw_on_error: true,
        show_passes: false,
        report_checks: false
      };
      return (new Test(guytest_cfg)).test(this.nfa_tasks);
    })();
  }

  // ( new Test guytest_cfg ).test @nfa_tasks.builtins

}).call(this);

//# sourceMappingURL=test-basics.js.map