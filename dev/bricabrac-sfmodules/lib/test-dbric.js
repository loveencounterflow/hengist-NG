...
$de );`);
    ...
    dbric_hoard_plugin_model: function() {
      var Hoard, Hoard_extras, Hoard_user, h, hoard_plugin, names, r, row, s, summarize_data, u, Ωbbdbr_278, Ωbbdbr_279, Ωbbdbr_280, Ωbbdbr_281, Ωbbdbr_282, Ωbbdbr_283, Ωbbdbr_284, Ωbbdbr_285, Ωbbdbr_287, Ωbbdbr_288;
...
values ( $rowid, $lo, $hi, $scatter );`
      ...
      Hoard_extras = class Hoard_extras extends Hoard {
        ...
        //-----------------------------------------------------------------------------------------------------
        save() {
          var data, hi, i, is_hit/* TAINT should be done by Hoard::create_scatter() */, j, len, len1, lo/* TAINT should be done by Scatter::create_run() */, r_idx, r_nr, r_rowid, ref, ref1, run, s_idx, s_nr, s_rowid, scatter;
          ref = this.scatters;
          for (s_idx = i = 0, len = ref.length; i < len; s_idx = ++i) {
            scatter = ref[s_idx];
            s_nr = s_idx + 1;
            s_rowid = `scatter,R=${s_nr}`;
            is_hit = from_bool(true);
            data = 'null';
            this.db.statements.hrd_insert_scatter.run({
              rowid: s_rowid,
              is_hit,
              data
            });
            ...
            for (r_idx = j = 0, len1 = ref1.length; j < len1; r_idx = ++j) {
              ...
              this.db.statements.hrd_insert_run.run({
                rowid: r_rowid,
                lo,
                hi,
                scatter: s_rowid
  ...
  demo_using_methods_holder_to_enable_ersatz_super = function() {
    ...
    A = class A {
      f(message) {
        return help('Ωbbdbr_291', rpr(message));
      }
    ...
    await (() => {
      ...
      if (do_coverage) {
        if (ca.unused_names.length > 0) {
          ...
          for (i = 0, len = ref.length; i < len; i++) {
            name = ref[i];
            warn('Ωbbdbr_294', "not covered:", reverse(name));
          }
        }
      }
